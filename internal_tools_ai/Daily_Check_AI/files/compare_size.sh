...
if [[ "$folder" == "saphir_prd" ]];then
      path_file_ytd=$(find ${target_dir}/${folder}/$(date --date=' 1 days ago' +'%Y%m') -name  "*$(date --date=' 1 days ago' +'%Y%m%d')*" )
      path_file_td=$(find ${target_dir}/${folder}/$(date +'%Y%m') -name  "*${TODAY}*" )
      echo "$path_file_ytd"
      echo "$path_file_td"
else
      path_file_ytd=$(find ${target_dir}/${folder} -name  *$(date --date=' 1 days ago' +'%Y%m%d')* )
      path_file_td=$(find ${target_dir}/${folder} -name "*${TODAY}*" )
      #path_file_td=$(find ${target_dir}/${folder} -name *$(date +'%Y%m%d')* )
      echo "$path_file_ytd"
      echo "$path_file_td"
fi
...
file_ytd=$(find ${target_dir}/${folder} -name  *$(date --date=' 1 days ago' +'%Y%m%d')* | awk -F/ '{print $NF}' )
file_td=$(find ${target_dir}/${folder} -name  "*${TODAY}*" | awk -F/ '{print $NF}' )
