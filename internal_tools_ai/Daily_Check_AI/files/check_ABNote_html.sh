...
if [[ "$sftp_error" -gt 0 ]];
 then
     ...
     sftp_status="<p>Check transfers to ABNotes: <span style="color:#F40C44">&nbsp;SFTP NOK&nbsp;</span></br><p>Please escalate Saas-team immediately to check error SFTP log on  MBEBKKSFTPP01"
 else
     sftp_status="<p>Check transfers to ABNotes: <span style="color:#05AD45">&nbsp;SFTP OK&nbsp;</span></br>"
fi

host=`hostname | awk -F. '{print $1}'`
echo "$sftp_status" >> /tmp/daily_check_$host-`date +"%Y%m%d"`.log
