...
#for BCIMR mostly false check on Friday morning
if [ "${bank}" == "BCIMR" ] ;  then
daily_file=$(ls -rt | tail -2 | grep "${bank}_account_output_D_" | tail -1)
# weekly_file=$(ls -rt | tail -2 | grep "${bank}_account_output_W_" | tail -1)
fi
