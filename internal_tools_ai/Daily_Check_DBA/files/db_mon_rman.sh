...
if [ ! -f "$RMAN_LOG_FILE" ] && [ "$DATE" == "Sat" ] && [ "$hostname" == "spfccdbbfcslp1" ] ;
then
    ## for BBF no rman backup on 03:00 Saturday.
    echo "
    <p><span style="font-size:14px"><b>************** List rman backup ${host} on ${hostname} ******************</b></span></br>
    <p>`date`</br>

   <p><b>Job rman archive log backup on ${host} <span style="color:#05AD45">&nbsp;skip on Saturday for BBF.&nbsp;</span></b></br> " > /tmp/daily_check_rman_$host-`date +"%Y%m%d"`.log
elif [ ! -f "$RMAN_LOG_FILE" ] ; then
    echo "
    <p><span style="font-size:14px"><b>************** List rman backup ${host} on ${hostname} ******************</b></span></br>
    <p>`date`</br>
