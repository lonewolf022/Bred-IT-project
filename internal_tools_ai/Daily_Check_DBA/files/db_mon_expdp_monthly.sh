...
DBNAME=$1
PREFIX_LOG=$3
DWH_MONTHLY_LOG=`ls -1 ${EXPDP_DIR} | grep ${PREFIX_LOG} | grep $MONTHLY_DATE | grep log`
...
then
   ...
   ************** List expdp monthly backup ${DBNAME} on ${hostname} ******************
   `date`
   Please call to DBA for check on ${hostname} !! No backup monthly log  ${MONTHLY_DATE} " > /tmp/monthly_check_expdp_$DBNAME-`date +"%Y%m%d"`.log
else
    ...
    RESULT_LOG=`less ${DWH_MONTHLY_LOG} | grep -i "Dump file set for" -A 10 -B 2 | sed 's/^/\<p>/' | sed 's/$/<\/br>/'`
        ...
        then
                ...
                ************** List expdp monthly backup ${DBNAME} on ${hostname} ******************
                `date`
                $RESULT_LOG
                <p> Job ${DBNAME} monthly export failed on $hostname" > /tmp/monthly_check_expdp_$DBNAME-`date +"%Y%m%d"`.log
        else
               rc=0
